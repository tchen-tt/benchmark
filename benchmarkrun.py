...
class Benchmark():
    def __init__(self, genomefa, tefa, tegtf, bwagenome, bowtiete, outputdir, threads, sConfig, tefamily, coverage, repeatsmask, index):
        # Comment
    ...
    def simulation(self):
        ...
        if stauts.returncode == 0:
            ...
            # rename
            input = pathlib.Path(self.insert, "1.modref.bed")
            output = pathlib.Path(self.insert, str(self.index)+"_"+str(self.coverate)+"x"+"_"+"1.modref.bed")
            self.run(f'mv {input} {output}')
    ...
    def TEi(self):
        ...
        self.run(f'rm {file}')
        # renames
        input = pathlib.Path(self.teiout, "te_insertion.bed")
        output = pathlib.Path(self.teiout, str(self.index)+"_"+str(self.coverate)+"x"+"_te_insertion.bed")
        self.run(f'mv {input} {output}')
    ...
    def neg_te_mapper2(self):
        ...
        # renames
        input = pathlib.Path(self.ngs_te_mapper2, "1.modref_2.nonref.bed")
        output = pathlib.Path(self.ngs_te_mapper2, str(self.index)+"_"+str(self.coverate)+"x"+"_1.modref_2.nonref.bed")
        self.run(f'mv {input} {output}')
    ...
    def RetroSeq(self, te_fasta_fastafile):
        ...
        self.run(commands)
        # rename
        input = pathlib.Path(self.retroseq, "retroseq.bed")
        output = pathlib.Path(self.retroseq, str(self.index)+"_"+str(self.coverate)+"x"+"_"+"retroseq.bed")
        self.run(f'mv {input} {output}')
    ...
    def TEMP2(self):
        ...
        if status.returncode == 0:
            rmtemp = pathlib.Path(self.temp2, "tmpTEMP2")
            self.run(f'rm -rf {rmtemp}')
            rmtemp = pathlib.Path(self.temp2, "temp.supportReads")
            self.run(f'rm -rf {rmtemp}')
            rmtemp = pathlib.Path(self.temp2, "temp.transposonMapping")
            self.run(f'rm -rf {rmtemp}')

        # rename
        input = pathlib.Path(self.temp2, "temp.insertion.bed")
        output = pathlib.Path(self.temp2, str(self.index)+"_"+str(self.coverate)+"x"+"_"+"temp.insertion.bed")
        self.run(f'mv {input} {output}')
...
def benchmark(genomefa: str, tefa: str, tegtf: str, bwagenome: str, bowtiete: str, outputdir: str, threads: int, sconfig: str, tefamily: str, coverage: int, repeatsmask: str, times: int) -> None:
    for i  in range(times):
        pip = Benchmark(genomefa=genomefa, tefa=tefa, tegtf=tegtf, bwagenome=bwagenome, bowtiete=bowtiete, outputdir=outputdir, threads=threads, sConfig=sconfig, tefamily=tefamily, coverage=coverage, repeatsmask=repeatsmask, index=i)
        pip.simulation()
        pip.bwa_mapping()
        pip.TEi()
        pip.neg_te_mapper2()
